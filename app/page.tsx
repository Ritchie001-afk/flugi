...
export default async function Home() {
  ...
  try {
    featuredDeals = await prisma.deal.findMany({
      ...
      // Ideally we would want OR: [{ expiresAt: { gt: new Date() } }, { expiresAt: null }],
      // but if Prisma schema has expiresAt as optional, we need to handle it.
      // Let's check schema first. If expiresAt is optional, we change logic.
      // Looking at schema, let's assume we want to show everything that hasn't expired yet OR has no expiration.
      where: {
      OR: [
